...
te = time.time()
print(infer_type, "not using batch", te - ts)
...
with open("test.txt", "r") as f:
    ...
    for line in lines:
        ...
        print("字数: ", len(text))
        t0 = time.time()
        outp = sambert_hifigan_tts_man.infer_batch(text=text, scale=1)
        t1 = time.time()
        print("----batch infer time: ", t1-t0)

te = time.time()
print(infer_type, "using batch", te-ts)
